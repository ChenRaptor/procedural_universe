...
//    // Nettoyage complet (couleur + profondeur)
//    glClearColor(0.1f, 0.1f, 0.2f, 1.f);
//    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
//    // --- 1. DESSIN DE LA PLANÈTE OPAQUE ---
//    planetShader->use();
//    glUniform3fv(glGetUniformLocation(planetShader->ID, "uLightDir"), 1, (float[]){0.f, 0.f, 1.f});
//    glUniform3f(glGetUniformLocation(planetShader->ID, "uCamPos"), camPosX, camPosY, camPosZ);
//    glUniformMatrix4fv(uModelLoc, 1, GL_FALSE, model);
//    glUniformMatrix4fv(uViewLoc, 1, GL_FALSE, view);
//    glUniformMatrix4fv(uProjectionLoc, 1, GL_FALSE, projection);
...
//    // Activation du test de profondeur et écriture activée
//    //glEnable(GL_DEPTH_TEST);
//    //glDepthMask(GL_TRUE);

//    planet->render();

//    // --- 2. PASS OIT : accumulation dans framebuffer ---
//    glBindFramebuffer(GL_FRAMEBUFFER, fbo);

//    // Nettoyer les deux textures d'accumulation (couleur et reveal)
//    glClearBufferfv(GL_COLOR, 0, (const GLfloat[]){0,0,0,0});
//    glClearBufferfv(GL_COLOR, 1, (const GLfloat[]){1.0f});

//    // Activer blending additif pour OIT
//    glEnable(GL_BLEND);
//    glBlendFunc(GL_ONE, GL_ONE);

//    // Maintenir le test de profondeur pour trier les fragments transparents,
//    // mais ne pas écrire dans le buffer profondeur pour éviter masquer la planète
//    glEnable(GL_DEPTH_TEST);
//    glDepthMask(GL_FALSE);
...
//    // Réactiver l’écriture de la profondeur pour la suite
//    glDepthMask(GL_TRUE);

//    glBindFramebuffer(GL_FRAMEBUFFER, 0);

//    // --- 3. PASS DE COMPOSITION FINALE ---
//    //glClearColor(0.1f, 0.1f, 0.2f, 1.0f);
//    //glClear(GL_COLOR_BUFFER_BIT);
...
//    // Activer un blending classique pour composer la transparence
//    glEnable(GL_BLEND);
//    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void render() {
    ...
    glEnable(GL_BLEND);
    // Blending classique pour superposer correctement la transparence sur la planète opaque
    //glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glBlendFunc(GL_ONE, GL_ONE);
